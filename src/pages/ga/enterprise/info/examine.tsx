  ...
  columns: StandardTableColumnProps[] = [
    {
      title: '企业名称',
      dataIndex: 'name',
      render: (v, r) => <Link to={`/ga/enterprise/examine/detail/${r.dm}`}>{v}</Link>,
    },
    {
      title: '统一社会信用代码',
      dataIndex: 'dm',
    ...
    },
    {
      ...
      render: (text, record) => (
        <Fragment>
          {record.status === 0 && (
            <Button onClick={() => this.handleUpdateModalVisible(true, record)}>审核</Button>
          )}
          {record.status === 1 && <Button>撤销</Button>}
        </Fragment>
      ),
    },
  ];
