  ...
  columns: StandardTableColumnProps[] = [
    {
      title: '投诉编号',
      dataIndex: 'no',
      render: (v, r) => <Link to={`/ga/complaint/enterprise/detail/${v}`}>{v}</Link>,
    },
    {
      title: '投诉企业名称',
      dataIndex: 'qy',
    ...
    },
    {
      ...
      render: (text, record) => (
        <Fragment>
          {record.status === 0 && (
            <Link to={`/ga/complaint/enterprise/detail/${record.no}?t=e`}>回复</Link>
          )}
          {record.status === 2 && <a>受理</a>}
        </Fragment>
      ),
    },
  ];
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          <Col md={8} sm={24}>
            <FormItem label="投诉企业">
              {getFieldDecorator('qy')(<Input style={{ width: '100%' }} />)}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="被投诉机构">
              {getFieldDecorator('jg')(<Input style={{ width: '100%' }} />)}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
